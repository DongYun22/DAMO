...
전체 스냅샷 방식(`write()`/`read()`)에 남아있는 12개 메서드 중 **사용 빈도가 높은 6개를 이번에 전환**한다.
...
- 이번에 전환하지 않는 나머지 6개 메서드는 계속 전역 락(`write()`)을 사용한다.
...
`voteResults`는 이미 `main`에서 SQL로 구현되어 있으므로(`postgres-store.ts:1949`), `closeVote`/`finalSelection`은 동점 판정 로직을 새로 짜지 않고 그 결과를 그대로 재사용한다.
...
- 검증 범위: 가입 코드 조회·가입, 모임 생성 시 호스트 멤버 동시 생성, 투표 생성 시 후보 고정과 세션 회전, 종료 시 단독/공동 1위 분기, 최종 선택, 정기 모임 다음 회차 자동 생성.

## 문서화

- README "현재 MVP에서 남은 작업" 목록의 "PostgreSQL 저장소의 나머지 변경 작업을 개별 SQL로 전환" 항목을 갱신 — 이번에 전환된 6개는 제외하고, 남은 6개(`leaveMeeting`, `kickMember`, `deleteMeeting`, `updateUserPlace`, `unregisterUserPlace`, `reset`)를 구체적으로 명시.
...
- `leaveMeeting`, `kickMember`, `deleteMeeting`, `updateUserPlace`, `unregisterUserPlace`, `reset`의 SQL 전환 (다음 사이클)
