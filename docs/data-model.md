...
- 변경 후 모임 목적과 성격이 모두 일치하지 않으면 해당 사용자의 추천을 후보에서 제거한다.
- 다른 추천자가 남아 있다면 통합 후보 자체는 유지한다.
- 추천자가 한 명도 남지 않으면 해당 후보를 제거한다.
...
```text
user_place.purpose = meeting.purpose
OR
user_place.mood = meeting.mood
```

- 조건 중 하나만 일치해도 후보 선택 목록에 표시한다.
