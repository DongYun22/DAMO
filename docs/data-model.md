...
- 일정이 지난 `RECRUITING`/`VOTING`/`FINAL_SELECTION` 모임은 서버가 실제로 `COMPLETED`로 전환한다. 진행 중이던 투표가 있었다면 함께 종료하고, 단독 1위가 있으면 그 후보를 최종 장소로 저장하며, 공동 1위이거나 아무도 투표하지 않았다면 최종 장소 없이 완료한다. 완료 이후에는 가입·후보 변경·투표 선택을 더 이상 진행할 수 없다.
- 이 전환은 해당 모임의 advisory lock 안에서 원자적으로 처리되며, 그 모임을 다루는 다음 요청(홈 조회 포함)이 트리거한다 — 별도의 배치/크론은 없다.
